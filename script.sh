...
# function to log messages
  ...
  #    echo $dbIp $dbPort $dbUsername $dbPassword $auddbName
    ...
    previousStatus=$(mysql -h$dbIp -P$dbPort $auddbName -u$dbUsername  -p${dbPassword} -se "select status from modules_audit_trail where feature_name='$featureName' and module_name='$moduleName and created_on like '%$(date +F)%' limit 1");
...
EOFMYSQL
    ...
    lastProcessedDate=$(mysql -h$dbIp -P$dbPort $auddbName -u$dbUsername -p${dbPassword} -se "select value from sys_param where tag=''"
    ...
    ipLocationDumpFileUrl=$(mysql -h$dbIp -P$dbPort $auddbName -u$dbUsername -p${dbPassword} -se "select value from sys_param where tag='ipLocationDumpFileURL'")
    ipLocationUrlKey=$(mysql -h$dbIp -P$dbPort $auddbName -u$dbUsername -p${dbPassword} -se "select value from sys_param where tag='ipLocationURLKey'")
    ipLocationCode=$(mysql -h$dbIp -P$dbPort $auddbName -u$dbUsername -p${dbPassword} -se "select value from sys_param where tag='ipLocationCode$ipType'")
    ...
    response=$(wget -O database.zip -q $ipLocationDumpFileURL 2>&1)
    ...
    fileName= "ip_location_country_ipv4_$(date +%Y%m%d).csv"
    fullFileName=$inputfilePath"/"$fileName
    takeDiff=1
    if [ -z "$lastProcessedDate" ];
      then
        log_message "The last processed date not exists. This is the first time processing. Taking current file as
        first dump to process."
        takeDiff=0
    fi
    ...
    if [ ! -s "$outputFileDeletion" && ! -s "$outputFileDeletion" ];
      then
    ...
    cd $javaProcessPath
    javaExitStatus=$(java -jar $build -Dlogging.config=./log4j2.xml --spring.config.location=./application.properties)
    javaFeatureName="$javaFeatureName_$ipType"
    ...
    if [ "$fileProcessStatusCode" -eq 200 ] ;
      then
        ...
        log_message "Remove the $sortedTempFile and $fullFileName"
        rm $fullFileName $sortedTempFile
        log_message "ReMove the remaining files from ${filePath}}"
        rm $inputfilePath/*
#        log_message "Moved file ${sortedTempFile} to ${processedFilePath}/${fileName}."
        ...
        log_message "Copied file $outputFileAddition to $deltaFileProcessedPath."
        ...
        generateAlertUsingUrl 'alert5311' $fileName $operator $alertUrl
        exit 1
    fi
  ...
  mysql -h$dbIp -P$dbPort -u$dbUsername -p${dbPassword} $auddbNam << EOFMYSQL
